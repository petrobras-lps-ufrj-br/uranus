...
def register_dashboard_callbacks(dash_app):
    ...
    @dash_app.callback(
        ...
        [Input("add-system-btn", "n_clicks"),
         ...
         Input({"type": "move-up-sys", "index": ALL}, "n_clicks"),
         Input({"type": "move-down-sys", "index": ALL}, "n_clicks")],
        State("systems-container", "children"),
        prevent_initial_call=True
    )
    def manage_systems(add_clicks, remove_clicks, up_clicks, down_clicks, current_systems):
        if current_systems is None: current_systems = []
        ctx = callback_context
        if not ctx.triggered: return current_systems
        trigger = ctx.triggered[0]['prop_id']
        
        if "add-system-btn" in trigger:
            try:
                db = SessionLocal()
                pis = db.query(PI).all()
        ...
        try:
            ...
            elif t_type == "move-up-sys" and pos > 0:
                current_systems[pos], current_systems[pos-1] = current_systems[pos-1], current_systems[pos]
            elif t_type == "move-down-sys" and pos < len(current_systems)-1:
                current_systems[pos], current_systems[pos+1] = current_systems[pos+1], current_systems[pos]
        except Exception as e:
    ...
    @dash_app.callback(
        ...
        [Input({"type": "add-sub-btn", "index": MATCH}, "n_clicks"),
         ...
         Input({"type": "move-up-sub", "index": ALL}, "n_clicks"),
         Input({"type": "move-down-sub", "index": ALL}, "n_clicks")],
        State({"type": "subsystems-container", "index": MATCH}, "children"),
        prevent_initial_call=True
    )
    def manage_subsystems(add_clicks, r_clicks, u_clicks, d_clicks, current_subs):
        if current_subs is None: current_subs = []
        ctx = callback_context
        if not ctx.triggered: return current_subs
        trigger = ctx.triggered[0]['prop_id']
        
        if "add-sub-btn" in trigger:
            try:
                new_sub_id = generate_id()
                db = SessionLocal()
                pis = db.query(PI).all()
                all_subs = db.query(Subsystem).all()
                db.close()
        ...
        try:
            ...
            elif t_type == "move-up-sub" and pos > 0:
                current_subs[pos], current_subs[pos-1] = current_subs[pos-1], current_subs[pos]
            elif t_type == "move-down-sub" and pos < len(current_subs)-1:
                current_subs[pos], current_subs[pos+1] = current_subs[pos+1], current_subs[pos]
        except Exception as e:
    ...
    @dash_app.callback(
        ...
        [Input({"type": "add-sub-sensor-btn", "index": MATCH}, "n_clicks"),
         ...
         Input({"type": "move-up-sensor", "index": ALL}, "n_clicks"),
         Input({"type": "move-down-sensor", "index": ALL}, "n_clicks")],
        State({"type": "sub-sensors-container", "index": MATCH}, "children"),
        prevent_initial_call=True
    )
    def manage_sensors(add_sub_clicks, r_clicks, u_clicks, d_clicks, sub_sensors):
        ...
        if "add-sub-sensor-btn" in trigger:
            new_id = generate_id()
            sub_sensors.append(render_sensor_form(new_id, pi_options))
            return sub_sensors
            ...
            for i, s in enumerate(sub_sensors):
                if isinstance(s, dict) and 'props' in s and 'id' in s['props'] and s['props']['id'] == {"type": "sensor-card", "index": idx}:
                    if t_type == "remove-sensor":
                        sub_sensors.pop(i)
                    elif t_type == "move-up-sensor" and i > 0:
                        sub_sensors[i], sub_sensors[i-1] = sub_sensors[i-1], sub_sensors[i]
                    elif t_type == "move-down-sensor" and i < len(sub_sensors)-1:
                        sub_sensors[i], sub_sensors[i+1] = sub_sensors[i+1], sub_sensors[i]
                    return sub_sensors
        except Exception as e:
    ...
    def save_entire_product(n_clicks, prod_id, p_name, p_desc, p_location, p_country, p_state, p_number, 
                           sys_names, sys_ids, sys_descs, 
                           sub_names, sub_ids, sub_descs, sub_parents_vals, sub_children_vals,
                           s_names, s_ids, s_tags, s_descs, s_pis):
        if not n_clicks: return ""
        try:
            db = SessionLocal()
            if prod_id:
    ...
    def update_display_sensors_table(prod_id):
        ...
        # Fetch components manually since relationships are gone
            ...
            for sub in subsystems:
                ...
                for s in sensors:
                    rows.append(html.Tr([
                        ...
                        html.Td(sub.name),
                        html.Td(", ".join(sub.parents) if sub.parents else "-"),
                        html.Td(", ".join(sub.children) if sub.children else "-"),
                        html.Td(s.description),
                        html.Td(s.pi_id or "N/A"),
                        html.Td(s.tag),
                    ]))
        db.close()
        ...
        table = dbc.Table([
            html.Thead(html.Tr([
                ...
                html.Th("Description"), html.Th("PI ID"), html.Th("Sensor Tag"),
            ])),
            html.Tbody(rows)
